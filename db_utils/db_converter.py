...
class DB_SCHEMA_GENERATOR:
    ...
    def __call__(self,
                 ...
                 user_email="harshithan@quinnox.com"):
        ...
        table_schema = self.mysql_adapter.execute_query(
            Queries.suite_schema.format(database_name=database_name,
                                        user_email=user_email))
        ...
        table_schemas += "TABLE NAME : test_suite " + table_schema[[
            'Field', 'Type'
        ]].to_csv(index=False)
        ...
        table_schema = self.mysql_adapter.execute_query(
            Queries.script_schema.format(database_name=database_name,
                                         user_email=user_email))
        table_schemas += "TABLE NAME : test_script " + table_schema[[
            'Field', 'Type'
        ]].to_csv(index=False)
        ...
        table_schema = self.mysql_adapter.execute_query(
            Queries.step_schema.format(database_name=database_name,
                                       user_email=user_email))
        table_schemas += "TABLE NAME : test_step " + table_schema[[
            'Field', 'Type'
        ]].to_csv(index=False)
        ...
        table_schema = self.mysql_adapter.execute_query(
            Queries.exec_schema.format(database_name=database_name,
                                       user_email=user_email))
        table_schemas += "TABLE NAME : test_execution " + table_schema[[
            'Field', 'Type'
        ]].to_csv(index=False)
        ...
        table_schema = self.mysql_adapter.execute_query(
            Queries.sprint_schema.format(database_name=database_name,
                                         user_email=user_email))
        table_schemas += "TABLE NAME : sprint " + table_schema[[
            'Field', 'Type'
        ]].to_csv(index=False)
        ...
        team_id = user_result['uuid'].tolist()
        print(team_id)
...
class SQL_CONVERTER:
    ...
    def __get_user_prompt__(self, user_description, db_schema, team_id
                            ):
        return MYSQL_CONVERTER_PROMPT.user_prompt.format(
    ...
    async def __call__(self,
        ...
        functions = [{
            ...
            "description": """This executes mysql commands.
                    """,
...
class CorrectQuery:
    ...
    def __call__(self, query, error, db_schema, database_name="webautomation"):
        ...
        messages = [{
            ...
            "user",
            "content":
            MYSQL_CORRECTOR_PROMPT.user_prompt.format(query=query,
                                                      error=error,
                                                      db_schema=db_schema)
        }]
        ...
        _, args = self.function_call(messages=messages,
