class MYSQL_CONVERTER_PROMPT:
    system_prompt = """You are an expert in MySQL database. User wants to interact with database in English. You will be a given the user_description and the database schema. Your job is to convert the `user_description` to MYSQL command. You will only support read commands i.e. select commands only. If the user description implies Delete, drop, alter, create etc., return empty string as response."""
    user_prompt = """user_description: {user_description}, database_schema: {db_schema}"""
...
class GENERATE_INSIGHT_LIST_PROMPT:
    ...
    Explore various aspects such as total counts, specific attributes, and other relevant metrics derived from the user's description than can be shown in analytics easily . Additionally, consider the relations present in the db_schema to formulate informed questions.Don't complex it ,keep it simple"""
    user_prompt = """db_schema: {db_schema}, user_description: {user_description}"""
